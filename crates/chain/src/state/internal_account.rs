...
pub fn create_account(mut state: &mut ChainState, addr: Address) {
...
pub fn increment_nonce(mut state: &mut ChainState, addr: &Address) {
let mut n = state.nonces.entry(*addr).or_insert(0u64);
*n = n.saturating_add(1);
}

pub fn mint(mut state: &mut ChainState, addr: &Address, amount: u128) -> Result<()> {
...
let mut bal = state.balances.entry(*addr).or_insert(0u128);
...
#[allow(dead_code)]
pub fn transfer_raw(mut state: &mut ChainState, from: &Address, to: &Address, amount: u128) -> Result<()> {
let mut from_bal = state.balances.entry(*from).or_insert(0u128);
if *from_bal < amount {
anyhow::bail!("insufficient funds")
}

*from_bal -= amount;
let mut to_bal = state.balances.entry(*to).or_insert(0u128);
*to_bal += amount;
Ok(())
}