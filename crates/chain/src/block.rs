...
Ok(h) => {
accum.extend_from_slice(h.as_bytes())
},
Err(_) => {
...
pub fn new(height: u64, parent_hash: Hash, txs: Vec<TxEnvelope>, state_root: Hash, proposer: Address, proposer_pubkey: Vec<u8>, gas_used: u64, gas_limit: u64) -> Self {
...
pub fn new_with_consensus(height: u64, parent_hash: Hash, txs: Vec<TxEnvelope>, state_root: Hash, proposer: Address, proposer_pubkey: Vec<u8>, gas_used: u64, gas_limit: u64, round_number: u64, proposer_index: u32, vrf_seed: Option<[u8; 32]>) -> Self {
let tx_root = Self::compute_tx_root(&txs);
let ts = Utc::now();
