...
use dsdn_common::{DAConfig, DAError, DAHealthStatus, DALayer, MockDA, BlobRef, BlobStream};
...
impl AppState {
    ...
    pub(crate) fn da(&self) -> &DARouter {
        &self.da_router
    }

    /// Check if DA is available based on current routing state.
    pub(crate) fn is_da_available(&self) -> bool {
    ...
    /// Get current routing state.
    pub(crate) fn routing_state(&self) -> RoutingState {
        self.da_router.current_state()
    }

    /// Get reference to health monitor.
    pub(crate) fn health_monitor(&self) -> &Arc<DAHealthMonitor> {
        self.da_router.health_monitor()
    }

    /// Get reference to reconciliation engine.
    pub(crate) fn reconciliation_engine(&self) -> &Arc<ReconciliationEngine> {
        &self.reconciliation_engine
    }
}

// ════════════════════════════════════════════════════════════════════════════
// MAIN - Delegates to cli module
// ════════════════════════════════════════════════════════════════════════════
