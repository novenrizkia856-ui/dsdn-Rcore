...
mod tests {
    ...
    fn sample_manifest_ok() -> Manifest {
        Manifest {
            ...
            entrypoint: "a3b1f9b0c8d4e5f6a7b8c9d0a1b2c3d4e5f6a7b8c9d0a1b2c3d4e5f6a7b8c9d".to_string(), // invalid length for safety; we'll replace in tests
            blobs: vec![],
            signatures: vec![Signature { key_id: "trusted-key".to_string(), sig: "signed".to_string() }],
            sbom: Some(SBOM { name: Some("demo".to_string()), license: Some("MIT".to_string()), supplier: None, extra: HashMap::new() }),
            meta: HashMap::new(),
        }
    }
    ...
    fn test_banned_hash_rejected() {
        ...
        let res = val.validate_manifest(&manifest).expect("validate");
        assert!(!res.ok);
    ...
    fn test_valid_manifest_passes_basic_checks() {
        ...
        let manifest = Manifest {
            ...
            sbom: Some(SBOM { name: Some("p".into()), license: Some("Apache-2.0".to_string()), supplier: None, extra: HashMap::new() }),
            meta: HashMap::new(),
        };
        let res = val.validate_manifest(&manifest).expect("validate");
